...
function visualizeEvents(data){
	...
	var _initTime = _nestTime[0].ts,
		_session = 0,
		_tempHold = _nestTime[0].ts;
	...
	_nestSessions.forEach(function(d){
		var _vals = d.values;
		var _max = d3.max(_vals,function(c){ return c.relts });
	...
	_nestSessions.forEach(function(d){
		var _vals = d.values;
		var _localMax = d3.max(_vals,function(c){ return c.relts });

		if(_localMax > 250){	
			_timeScale.domain([0,_localMax]);

			_vals.forEach(function(g){
	...
	var _newData = _nestTime.slice(0);
